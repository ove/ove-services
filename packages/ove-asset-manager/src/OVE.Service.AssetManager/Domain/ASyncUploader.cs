...
namespace OVE.Service.AssetManager.Domain {
    public class ASyncUploader : IHostedService, IDisposable {
        ...
        public Task StartAsync(CancellationToken cancellationToken) {
            ...
            _timer = new Timer(RunProcessing, null, TimeSpan.Zero,
        ...
        public Task StopAsync(CancellationToken cancellationToken) {
            _logger.LogInformation("Async uploader Service is stopping.");
        ...
        public static async void DeleteAssetModel(OVEAssetModel todo, IConfiguration configuration) {
        ...
        private async void RunProcessing(object state) {
                ...
                using (var context = scope.ServiceProvider.GetRequiredService<AssetModelContext>()) {
                    ...
                    try {
                        ...
                        if (todo == null) {
                            ...
                            _logger.LogInformation("Starting uploading of file id=" + todo.Id);

                            //figure out where the file is 
                            var file = Path.Combine(OVEAssetModel.GetAssetsBasePath(_configuration, _logger),
                                todo.Project);
                            file = Path.Combine(file, todo.StorageLocation);

                            if (!File.Exists(file)) {
                                throw new Exception("Found an orphan database model - no corresponding file " + todo.Id);
                            }
